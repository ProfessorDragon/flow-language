...
setblocks = ["var_set", "var_increment", "array_replace", "dict_set", "comment"]
builtins = []
data = []
scripts = [{"name": "main", "blocks": [["script_start"]]}]
script = 0
modules = []
functionalities = {"console": True, "autoinstall": True, "windows": True, "modernwindows": True, "random": True, "json": True,
                   "keyboard": False, "automation": False, "playsound": False, "tts": False}
...
funcscaffold = {"console": {"name": "Console"},
                ...
                "windows": {"name": "Windows + canvas", "imports": """\
import tkinter as tk
import tkinter.messagebox as tkmb
import tkinter.simpledialog as tksd
import tkinter.filedialog as tkfd""", "functions": """\
...
def _tk_packed(wid):
    wid.pack()
    return wid""", "init": """\
tkvars = {'window': None, 'canvas': None, 'fill': 'white', 'outline': 'black', 'outlinewidth': 1, 'font': 'Verdana 20'}""", "globals": ["tk", "tkmb", "tksd", "tkfd", "tkvars"]},
                "modernwindows": {"name": "Modern windows", "imports": "from tkinter.ttk import*"},
                "random": {"name": "Randomization", "imports": "import random", "globals": ["random"]},
                "json": {"name": "JSON", "imports": "import json", "globals": ["json"]},
                "keyboard": {"name": "Keyboard + mouse", "imports": "import keyboard, mouse", "modules": ["keyboard", "mouse"], "functions": """\
def _kb_press_and_release(name, delay):
    keyboard.press(name)
    time.sleep(delay)
    keyboard.release(name)"""},
                "automation": {"name": "Automation", "imports": "import pyautogui as pag", "modules": ["pyautogui"], "functions": """\
def _pag_colors_match(a, b, tolerance):
    return abs(a[0]-b[0]) <= tolerance and abs(a[1]-b[1]) <= tolerance and abs(a[2]-b[2]) <= tolerance""", "globals": ["pag"]},
...
def drawblock(b, x, y, cat=None, menu=False, absolute=False, preview=None, color=None, notext=False, tags=None):
    ...
    if blockinfo.get("requiremodule") is not None and not functionalities.get(blockinfo["requiremodule"], False):
        color = "error"
    elif cat.get("requiremodule") is not None:
        require = cat["requiremodule"]
        if type(require) == str: require = [require]
        if all(not functionalities.get(module, False) for module in require): color = "error"
...
def updateblockbar(e=None):
    ...
    for i, c in enumerate(categories):
        if c.get("hidden", False):
            continue
        if c.get("requiremodule") is not None:
            require = c["requiremodule"]
            if type(require) == str: require = [require]
            if all(not functionalities.get(module, False) for module in require): continue
        cc = category == i
    ...
    for i, b in enumerate(categories[category]["blocks"]):
        ...
        args = 0
        for a in getblock(b).get("args", []):
            if a[0] != "c": args += 1
        if args == 0:
            blockbar.create_text(bbox[2]+15, bbox[1]+(bbox[3]-bbox[1])/2-4, text=".", anchor=W, font=f"{font} {fontsize+13} bold")
        y += bbox[3]-bbox[1]+18
...
def updatescriptbar(e=None):
    ...
    comprect = scriptbar.create_rectangle(55, y+10, 91, y+40, fill="royalblue")
    compcirc = scriptbar.create_oval(66, y+18, 80, y+32, fill="lightgray", outline="")
    for obj in [comprect, compcirc]:
    ...
    for i, s in enumerate(scripts):
        ...
        y += 38
    plusrect = scriptbar.create_rectangle(10, y, 91, y+30, fill="white")
    plusver = scriptbar.create_line(50.5, y+5, 50.5, y+26, fill="gray", width=4)
    plushor = scriptbar.create_line(41, y+15, 60, y+15, fill="gray", width=4)
    for obj in [plusrect, plusver, plushor]:
        scriptbar.tag_bind(obj, f"<{btnmode}-1>", newscript, False)
...
def blockctrlclick(path, num, e=None, fixed=False, cpart=0, arg=False):
    ...
    if fixed:
        ...
        return
    if len(block) > 0:
        if "c" in [a[0] for a in getblock(path[num][0]).get("args", [])]: return
        if block[0][0] in setblocks or path[num][0] in setblocks: return
        n = 0
...
def loadextensions():
    ...
    with open(os.path.join(os.path.dirname(__file__), "flow_data.json")) as f:
        jsontovars(json.load(f))
    for p in extpaths:
        try:
            with open(p) as f:
                jsontovars(json.load(f), merge=True)
        except Exception as e:
                showerror("Load extensions", f"Error loading extension {os.path.basename(p)}:\n{e}")
    builtins = ["os", "sys", "math", "time", "subprocess", "traceback", "literal_eval", "autoinstall"]
...
def savesettings():
    with open("flow_settings.json", "w") as f:
        json.dump(varstojson("font", "fontbig", "fontsize", "fonty", "blocksize", "scrollspeed", "saveloc", "previewmode",
                             "argcolor", "varargcolor", "scriptargcolor", "ignorewarnings", "shortmode", "backpack", "extpaths"), f)
...
def blockhelp(b=None, e=None):
    ...
    i = getblock(b)
    showinfo(i["text"], i.get("help", "Help not available."))
...
def play(run, e=None, debug=False):
    ...
    def blockchain(array, i, c, indents=0, arg=False, globs=[], addglobs=False):
        ...
        if debug: print(b[0])
        if "scaffold" in info:
            try: tmp = info["scaffold"].format(*args)
            except IndexError: print("ERROR WITH BLOCK:", info)
...
"""
        ...
        if functionalities["autoinstall"]:
            if not functionalities["console"] or functionalities["windows"]:
                if not functionalities["windows"]:
                    c += "import tkinter.messagebox as tkmb\n"
                c += f"""\
def _flow_autoinstall():
    if tkmb.askyesno("Modules missing", f"Critical error - missing module(s) {{', '.join(autoinstall)}}. This program will not function without them. \\
Would you like to install them now?"):
        for mod in autoinstall:
            subprocess.check_call(["python", "-m", "pip", "install", mod])
        tkmb.showinfo("Modules installed", "All modules successfully installed! Restart the program to continue.")
        exit()
"""
            else:
                c += f"""\
...
def _flow_joinadd(p1, p2):
    if type(p1) == str or type(p2) == str:
        return str(p1)+str(p2)
    return p1+p2
def _flow_safediv(p1, p2):
    if p2 == 0: return 0
    return p1/p2
...
editmenu.add_command(label="New script", accelerator="Ctrl+Shift+N", command=newscript)
root.bind("<Control-N>", newscript)
...
helpmenu.add_command(label="Open temp directory", command=opentemploc)
