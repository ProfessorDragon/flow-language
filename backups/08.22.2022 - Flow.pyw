...
functionalities = {"console": True, "autoinstall": True, "windows": True, "dialogs": True, "modernwindows": True, "random": True, "time": True, "json": True,
                   "keyboard": False, "automation": False, "playsound": False, "tts": False, "translate": False}
...
def updateblockbar(e=None):
    ...
    for i, b in enumerate(categories[category]["blocks"]):
        ...
        for a in getblock(b).get("args", []):
            if a[0] != "c": args += 1
        if args == 0:
            blockbar.create_text(bbox[2]+15, bbox[1]+(bbox[3]-bbox[1])/2, text=f"X", anchor=W, font=f"{font} {fontsize+13}")
        y += bbox[3]-bbox[1]+18
